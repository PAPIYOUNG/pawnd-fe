...
/**
 * PostsFilterBar (Client Component)
 * - แถบค้นหา + ตัวกรองประเภท/สถานะ สำหรับตาราง "จัดการประกาศ"
 * - เมื่อเปลี่ยนค่า จะ sync กลับไปที่ query string ของหน้า (?search=&type=&status=)
...
export function PostsFilterBar({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-3xl border border-border bg-card p-4 sm:flex-row sm:items-center">
      {/* ช่องค้นหาด้วยหัวข้อประกาศ / ชื่อเจ้าของ / สถานที่ */}
      <div className="relative flex-1">
        ...
        <input
          ...
          placeholder="ค้นหาด้วยหัวข้อประกาศ, ชื่อเจ้าของ, สถานที่..."
      ...
      {/* ตัวกรองประเภทประกาศ: สัตว์เลี้ยงหาย / พบสัตว์เลี้ยง */}
      <select
        aria-label="กรองตามประเภทประกาศ"
      ...
      >
        ...
        {(Object.keys(POST_TYPE_LABEL) as PostType[]).map((value) => (
          <option key={value} value={value}>
            ประเภท: {POST_TYPE_LABEL[value].text}
          </option>
        ))}
      </select>
      ...
      >
        ...
        {(Object.keys(POST_STATUS_LABEL) as PostStatus[]).map((value) => (
          <option key={value} value={value}>
            สถานะ: {POST_STATUS_LABEL[value].text}
          </option>
        ))}
      </select>
    </div>
